-- KO'AKE Event Photo v1.4.2 — unique-to-the-satang order amounts
-- Run after 20260728_slip_hash_dedupe_v141.sql. Safe on an existing database.
--
-- Adds 1-99 random satang (0.01-0.99 baht) on top of the listed tier price when an
-- order is created, using Postgres core random() (no pgcrypto dependency). Every
-- order then has a near-unique exact amount, so the photographer can match a slip
-- to the right order at a glance instead of many orders sharing a round number like
-- 100.00 — this is a free, no-API way to make slip mix-ups and reuse much easier to
-- catch by eye, on top of the file-hash duplicate check already in place.
...
begin
  ...
  v_amount_satang := v_tier.price_satang + (1 + floor(random() * 99))::integer;

  insert into public.event_photo_orders(event_id, order_number, buyer_phone, selected_count, amount_satang)
  values(v_event.id, v_order_number, nullif(trim(p_buyer_phone), ''), v_count, v_amount_satang)
