...
// NOTE: unlike the verify-slip contract (confirmed against real dashboard
// docs + a live test), this account/info shape is only a best guess based on
// the endpoint URL shown as an *example* on Slip2Go's own Authentication page
// (`GET {apiUrl}/api/account/info`). The exact field names for remaining
// quota were never confirmed against real docs. `raw` always carries the full
// parsed response through untouched so an admin (or a future fix) can see
// exactly what came back even if the guessed field names below are wrong.
export async function getSlip2GoQuota(): Promise<Slip2GoQuota> {
  const secretKey = process.env.SLIP2GO_SECRET_KEY?.trim()
  if (!secretKey) return { httpStatus: 0, raw: null, remaining: null, limit: null }
  try {
    ...
    return {
      ...
      raw,
      remaining: pick(['remaining', 'remainingToken', 'remainingQuota', 'balance', 'credit', 'token', 'quotaRemaining']),
      limit: pick(['limit', 'quota', 'totalQuota', 'maxToken', 'quotaLimit']),
    }
  } catch (error) {
    return { httpStatus: 0, raw: null, remaining: null, limit: null, networkError: error instanceof Error ? error.message : 'NETWORK_ERROR' }
  }
}
