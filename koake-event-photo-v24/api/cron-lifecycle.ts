...
import { getSlip2GoQuota } from '../server/slipVerification.js'
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    // Best-effort daily Slip2Go quota check. Never lets a quota-check problem
    // fail the actual lifecycle cron — this whole block is advisory only.
    // Field names for "remaining" aren't confirmed against real Slip2Go docs
    // (see getSlip2GoQuota's own comment), so this stays silent unless it got
    // a number it's reasonably confident in.
    try {
      const quota = await getSlip2GoQuota()
      const lowOnCount = quota.remaining !== null && quota.remaining <= 10
      const lowOnRatio = quota.remaining !== null && quota.limit !== null && quota.limit > 0 && quota.remaining / quota.limit <= 0.2
      if (lowOnCount || lowOnRatio) {
        await notifyTelegram(
          `⚠️ โควต้าตรวจสลิปอัตโนมัติ (Slip2Go) เหลือน้อย\nคงเหลือ ${quota.remaining}${quota.limit !== null ? ` / ${quota.limit}` : ''}\nถ้าใช้หมด ระบบจะยังทำงานปกติแต่ไม่อนุมัติอัตโนมัติ ตกเป็นตรวจด้วยมือแทนโดยอัตโนมัติ — เติมโควต้าได้ที่แดชบอร์ด Slip2Go`,
        )
      }
    } catch (error) {
      console.error('SLIP2GO_QUOTA_CHECK_FAILED', error)
    }
