...
import { getSlip2GoQuota, getSlip2GoSecretDiagnostics, probeSlip2GoAuth } from '../server/slipVerification.js'
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  // Admin-only diagnostic for the Slip2Go auto-approval integration. Reports
  // whether the runtime actually sees a usable SLIP2GO_SECRET_KEY (length,
  // stray whitespace/newline/quotes, a short fingerprint — never the real
  // value) and, optionally, a live auth-only probe against Slip2Go so a
  // secret-vs-account problem can be told apart from a code problem without
  // ever screenshotting the real key again.
  if (queryValue(req.query?.diagnostic) === 'slip2go') {
    try {
      const secret = getSlip2GoSecretDiagnostics()
      const shouldProbe = queryValue(req.query?.probe) === '1'
      const [probe, quota] = shouldProbe
        ? await Promise.all([probeSlip2GoAuth(), getSlip2GoQuota()])
        : [null, null]
      console.info('SLIP2GO_AUTH_DIAGNOSTIC', { ...secret, probe, quota })
      return res.status(200).json({ ...secret, probe, quota })
    } catch (error) {
      return res.status(500).json({ error: error instanceof Error ? error.message : 'SLIP2GO_DIAGNOSTIC_FAILED' })
    }
  }
