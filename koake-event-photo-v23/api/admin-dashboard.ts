...
import { getSlip2GoSecretDiagnostics, probeSlip2GoAuth } from '../server/slipVerification.js'
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (queryValue(req.query?.diagnostic) === 'slip2go') {
    try {
      ...
      const probe = shouldProbe ? await probeSlip2GoAuth() : null
      console.info('SLIP2GO_AUTH_DIAGNOSTIC', { ...secret, probe })
      return res.status(200).json({ ...secret, probe })
    } catch (error) {
